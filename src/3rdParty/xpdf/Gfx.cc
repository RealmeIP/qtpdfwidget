...
void Gfx::doPatternFill(GBool eoFill) {
  ...
  //vishwajeet type casting added
  xi0 = (int)floor(float(xMin / xstep));
  xi1 = (int)ceil(float(xMax / xstep));
  yi0 = (int)floor(float(yMin / ystep));
  yi1 = (int)ceil(float(yMax / ystep));
  for (i = 0; i < 4; ++i) {
    m1[i] = m[i];
  }
...
void Gfx::doRadialShFill(GfxRadialShading *shading) {
  ...
  if (t < 1) {
    n = 3;
      ...
      //vishwajeet typecasting added
    n = (int)(M_PI / acos(float(1 - 0.1 / t)));
    if (n < 3) {
      n = 3;
    } else if (n > 200) {
      n = 200;
  ...
  while (ia < radialMaxSplits) {
    ...
    for (k = 1; k < n; ++k) {
	  ...
	  float tempcos = cos((float)angle);
	  float tempsin = sin((float)angle);
      state->lineTo(xa + ra * (double)tempcos, ya + ra * (double)tempsin);
    }
    state->closePath();
    ...
    for (k = 1; k < n; ++k) {
	  ...
	  float tempcos = cos((float)angle);
	  float tempsin = sin((float)angle);
      state->lineTo(xb + rb * (double)tempcos, yb + rb * (double)tempsin);
    }
    state->closePath();
