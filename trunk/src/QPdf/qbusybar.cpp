...
QBusyBar::QBusyBar ( QWidget *parent, const char *name, int flags ) : QWidget( parent,Qt::Widget)
{
	m_busy = 0;
...
{	
	...
	QRgb rgb = this->palette().color(QPalette::Highlight).rgb();

	rf = qRed(rgb);
	gf = qGreen(rgb);
	bf = qBlue(rgb);

	rgb = this->palette().color(QPalette::Background).rgb();
